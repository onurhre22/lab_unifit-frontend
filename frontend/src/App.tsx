...
export default function App() {
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      {/* ✅ HERO: Clova 느낌(중앙 집중 + 장식 요소 + 스크롤 유도) */}
      <section className="relative overflow-hidden bg-gradient-to-b from-indigo-500 via-indigo-400 to-slate-50">
        {/* 배경 장식(오른쪽 이미지 느낌: 블러/오브젝트) */}
        <div className="pointer-events-none absolute inset-0">
          <div className="absolute -top-24 left-1/2 h-72 w-72 -translate-x-1/2 rounded-full bg-white/20 blur-3xl" />
          <div className="absolute right-[-80px] top-24 h-72 w-72 rounded-full bg-fuchsia-200/20 blur-3xl" />
          <div className="absolute left-[-90px] bottom-16 h-80 w-80 rounded-full bg-cyan-200/15 blur-3xl" />
          <div className="absolute left-1/2 top-40 h-2 w-2 -translate-x-1/2 rounded-full bg-white/50 shadow-[0_0_40px_8px_rgba(255,255,255,0.35)]" />
        </div>
          ...
          <section id="services">
            ...
            <div className="mt-4">
              ...
              <div className="flex gap-4 overflow-x-auto pb-2 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden md:grid md:grid-cols-4 md:overflow-visible md:pb-0">
                <ServiceCard
                  title="AI 강좌 로드맵"
                  desc="진로/역량 기반으로 학기별 추천 로드맵을 제안합니다."
                  badge="추천"
                />
                <ServiceCard
                  title="팀 프로젝트 매칭"
                  desc="역량/성향/시간표 기반으로 최적 팀 구성을 추천합니다."
                />
                <ServiceCard
                  title="선배 멘토링"
                  desc="관심 분야에 맞는 선배를 찾아 1:1 상담을 연결합니다."
                />
                <ServiceCard
                  title="AI 포트폴리오"
                  desc="업로드만 하면 자동 정리·피드백까지 제공해요."
                />
              </div>
            </div>
          </section>
